...
import edu.ehu.galan.lite.utils.ixatools.pos.lemma.DictionaryLemmatizer;
import edu.ehu.galan.lite.utils.ixatools.pos.lemma.MorfologikLemmatizer;
import edu.ehu.galan.lite.utils.ixatools.pos.lemma.MultiWordMatcher;
import edu.ehu.galan.lite.utils.ixatools.pos.pos.dict.DictionaryTagger;
import edu.ehu.galan.lite.utils.ixatools.pos.pos.dict.MorfologikMorphoTagger;

/**
 * Main annotation class of ixa-pipe-pos. Check this class for examples using the ixa-pipe-pos API.
 *
 * @author ragerri
 * @version 2014-12-05
 */
public class Annotate {
    ...
    /**
     * The directory where the dictionaries and models will be searched
     */
    ...
    /**
     * Load the binary lemmatizer dictionaries by language. Exits if no lemmatizer dictionary
     * (binary) is available for the input language.
     *
    ...
    /**
     * Load the pos tagger dictionaries by language and format. Exits if no pos tagger dictionary
     * (binary) is available for the input language.
     *
     * @param props the props object
     */
    private void loadMorphoTaggerDicts(final Properties props) {
        final Resources resources = new Resources();
        final URL binDictMorphoTaggerURL;
        if (directory == null) {
            binDictMorphoTaggerURL = resources.getBinaryTaggerDict(this.lang);
        } else {
            binDictMorphoTaggerURL = resources.getBinaryTaggerDictFromDir(directory, this.lang);
        }
    ...
    /**
     * Creates the multiword spans. It gets an initial list of spans (one per token) and creates a
     * multiword span when a multiword is detected.
     *
     * @param tokens the list of tokens
     * @param wfs the list of WFs
     * @param tokenSpans the list of initial token spans
     */
